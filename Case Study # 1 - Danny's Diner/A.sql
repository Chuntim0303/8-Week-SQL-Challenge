...
SELECT
  f.customer_id,
  f.product_name AS first_purchased_item
FROM
  first_purchase_cte f
WHERE
  f.purchase_rank = 1;
  
-- 4. What is the most purchased item on the menu and how many times was it purchased by all customers?
SELECT m.product_id, m.product_name, COUNT(*) AS purchase_count
FROM sales s
JOIN menu m ON s.product_id = m.product_id
...
-- 5. Which item was the most popular for each customer?
SELECT t.customer_id, m.product_name, t.purchase_count
FROM (
...
-- 6. Which item was purchased first by the customer after they became a member?
#  SELECT m.product_name AS first_item_purchased, s.customer_id
...
WHERE
  (s.order_date >= '2021-01-01' AND s.order_date <= '2021-01-31') -- Filter sales in January
  AND s.customer_id IN ('A', 'B') -- Filter customers A and B
GROUP BY
  s.customer_id;
